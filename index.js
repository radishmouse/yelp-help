...
app.get('/:phonenumber/:apiKey', (req, res) => { // everything after :port is known as "phonenumber"

    const client = yelp.client(req.params.apiKey); 

    client.phoneSearch({ // that's from way above, the yelp-fusion library (happens inside of a callback: when you see ":port/stuff", run this)
